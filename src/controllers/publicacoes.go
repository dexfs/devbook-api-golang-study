...
func AtualizarPublicacao(w http.ResponseWriter, r *http.Request) {
	...
	defer db.Close()
	repositorio := repositorios.NovoRepositorioDePublicacoes(db)
	publicacaParaAtualizar, erro := repositorio.BuscarPorID(publicacaoID)
	if erro != nil {
		respostas.Erro(w, http.StatusInternalServerError, erro)
		return
	}

	if publicacaParaAtualizar.AuthorID != usuarioID {
...
func DeletarPublicacao(w http.ResponseWriter, r *http.Request) {
	...
	defer db.Close()
	repositorio := repositorios.NovoRepositorioDePublicacoes(db)
	publicacaParaDeletar, erro := repositorio.BuscarPorID(publicacaoID)
	if erro != nil {
		respostas.Erro(w, http.StatusInternalServerError, erro)
		return
	}

	if publicacaParaDeletar.AuthorID != usuarioID {
		respostas.Erro(w, http.StatusForbidden, errors.New("Não é possível atualizar uma publicação que não seja sua"))
		return
	}
...
func CurtirPublicacao(w http.ResponseWriter, r *http.Request) {
	...
	defer db.Close()
	repositorio := repositorios.NovoRepositorioDePublicacoes(db)
	erro = repositorio.CurtirPublicacao(publicacaoID)
	if erro != nil {
		respostas.Erro(w, http.StatusInternalServerError, erro)
		return
	}
...
func DescurtirPublicacao(w http.ResponseWriter, r *http.Request) {
	...
	defer db.Close()
	repositorio := repositorios.NovoRepositorioDePublicacoes(db)
	erro = repositorio.DescurtirPublicacao(publicacaoID)
	if erro != nil {
		respostas.Erro(w, http.StatusInternalServerError, erro)
		return
	}
